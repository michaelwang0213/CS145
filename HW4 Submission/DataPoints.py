...
class DataPoints:
    ...
    def getMean(clusters, mean):
        ...
        for i in range(len(clusters)):
            for point in clusters[i]:
                mean[i][0] += point.x
                mean[i][1] += point.y
            mean[i][0] /= len(clusters[i])
            mean[i][1] /= len(clusters[i])
    ...
    def getStdDeviation(clusters, mean, stddev):
        ...
        for i in range(len(clusters)):
            for point in clusters[i]:
                stddev[i][0] += (point.x - mean[i][0]) ** 2
                stddev[i][1] += (point.y - mean[i][1]) ** 2
            stddev[i][0] /= len(clusters[i])
            stddev[i][0] = (stddev[i][0]) ** .5
            stddev[i][1] /= len(clusters[i])
            stddev[i][1] = (stddev[i][1]) ** .5
    # -------------------------------------------------------------------
    @staticmethod
    def getCovariance(clusters, mean, stddev, cov):
        ...
        for i in range(len(clusters)):
            for point in clusters[i]:
                cov[i][0][1] += (point.x - mean[i][0])*(point.y - mean[i][1])
            cov[i][0][0] = stddev[i][0] ** 2
            cov[i][1][1] = stddev[i][1] ** 2
            cov[i][0][1] /= len(clusters[i])
            cov[i][1][0] = cov[i][0][1]
    # -------------------------------------------------------------------
    @staticmethod
