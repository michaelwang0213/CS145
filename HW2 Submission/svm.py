...
class SVM(object):
    ...
    def project(self, X):
        if self.w is not None: ## linear case
            predict_y = 0
            ########## Please Fill Missing Lines Here ##########
            predict_y = np.dot(X, (self.w).T) + self.b
            ...
            for i in range(len(self.a)):
                ...
                if self.kernel == gaussian_kernel:
                    xiTx = gaussian_kernel_point(X,self.sv[i])
                else:
                    xiTx = polynomial_kernel_point(X,self.sv[i])

                y_predict += self.a[i] * self.sv_y[i] * xiTx

                #y_predict += xtreme*self.a[i]*self.sv_y[i]*xiTx
