...
def getBeta(train_x, train_y):   
    ...
    XTXI = np.linalg.inv(np.dot(train_x.T, train_x))
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
       ...
       for i in range(n):
           ########## Please Fill Missing Lines Here ##########
           dJi = np.dot(train_x[i], np.dot(train_x[i].T, beta) - train_y[i])
           deriv = deriv + dJi
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            ########## Please Fill Missing Lines Here ##########
            dJi = np.dot(train_x[idx], train_y[idx] - np.dot(train_x[idx].T, beta))
            beta = beta + dJi.dot(lr/10)
    return beta
