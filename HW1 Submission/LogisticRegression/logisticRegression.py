...
def compute_avglogL(X,y,beta):
    ...
    avglogL = 0
    for i in range(n):
        avglogL += np.dot(y[i], np.dot(X[i].T, beta)) - np.log(1 + eps + np.exp(np.dot(X[i].T,beta)))
    avglogL /= n
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
            piB = sigmoid(np.dot(train_x[i].T, beta))
            for j in range(p):
                dlogLB[j] += train_x[i][j] * (train_y[i] - piB)

        beta = beta + dlogLB.dot(lr)
...
# lr (learning rate) is a scalar
# function returns value of beta calculated using (0) batch gradient descent
def getRegularized(train_x, train_y, lr, num_iter, verbose):
    beta = np.random.rand(train_x.shape[1])
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
            ...
            for j in range(p):
                dlogLB[j] += train_x[i][j] * (train_y[i] - piB)
                for k in range(p):
                    d2logLB[j][k] -= train_x[i][j]*train_x[i][k]*piB*(1-piB)
                    
        beta = beta - np.dot(np.linalg.inv(d2logLB + np.identity(p).dot(1e-50)), dlogLB)
...
class LogisticRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            beta = getRegularized(newTrain_x, self.train_y.values, self.lr, self.num_iter, self.verbose)
            print('Beta: ', beta)
        else:
